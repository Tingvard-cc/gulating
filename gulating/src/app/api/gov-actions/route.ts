...
async function storeSignatureOnDisk(
    ...
    // atomic write; if exists, append timestamp to avoid overwrite
    try {
...
/* ------------------------------------------------------------------ *
 * Koios request helper with retries/backoff/jitter
 * ------------------------------------------------------------------ */
async function koiosRequest(
    url: string,
    {
        attempts = 4,           // total tries
        timeoutMs = 12_000,     // per-attempt timeout
    ...
    for (let i = 0; i < attempts; i++) {
        ...
        try {
            ...
            const text = await res.text(); // read once for better diagnostics
...
/* ------------------------------------------------------------------ *
 * GET: (A) download .witness  (B) votes (uncached)  (C) proposals (Koios)
 * ------------------------------------------------------------------ */
export async function GET(req: Request) {
    const { searchParams } = new URL(req.url);

    // (A) Download stored file — keep **uncached**
    ...
    // (B) GET votes for a proposal — keep **uncached**
    ...
    // (C) Fetch proposals from Koios — **resilient + cacheable**
    try {
        const KOIOS =
        ...
        // get current epoch
        const tipRes = await koiosRequest(`${KOIOS}/tip`, { headers: extraHeaders });
        if (!tipRes.ok) {
            return NextResponse.json(
                {
        ...
        if (!Number.isFinite(currentEpoch)) {
            return NextResponse.json(
                { error: "Could not determine current epoch from Koios /tip", tipJson: tipRes.json },
                { status: 502 }
            );
        }
...
/* ------------------------------------------------------------------ *
 * POST: (A) store signature  (B) save votes
 * ------------------------------------------------------------------ */
