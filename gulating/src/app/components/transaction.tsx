      ...
      if (votingProcedures){
        ...
        for (const vote of votes){
          ...
          const voteChoice = (vote.voting_procedure.vote === 'Yes' ? 'Constitutional' : vote.voting_procedure.vote === 'No' ? 'Unconstitutional' : 'Abstain');
            let metadataURL = vote.voting_procedure.anchor?.anchor_url ?? "unavailable";
            const metadataHash = vote.voting_procedure.anchor?.anchor_data_hash ?? "unavailable";

            // FIX THE BAD URL: Check for the incorrect format and change it back
            if (metadataURL.startsWith("https://ipfs/")) {
                metadataURL = metadataURL.replace("https://ipfs/", "ipfs://");
            }  
