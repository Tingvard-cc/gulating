...
/**
 * Simple client-side password gate for the Files tab.
 * Reads password from process.env.NEXT_PUBLIC_FILES_PASSWORD
 * (you’re loading this from .env.example via your dotenv setup).
 */
function PasswordProtectedPanel({
    storageKey = "filesUnlocked",
    children,
}: {
    storageKey?: string;
    children: React.ReactNode;
}) {
    const requiredPassword = process.env.NEXT_PUBLIC_FILES_PASSWORD ?? "";
    ...
    const handleUnlock = () => {
        setError(null);
        if (!requiredPassword) {
            setError("No password is configured. Set NEXT_PUBLIC_FILES_PASSWORD in your env.");
            return;
        }
    ...
    if (!isUnlocked) {
        return (
            <Paper elevation={0} sx={{ p: 3, borderRadius: 2, textAlign: "center" }}>
                <Lock sx={{ fontSize: 48, mb: 1, opacity: 0.8 }} />
...
export default function Home() {
    ...
    const handleWalletConnectionChange = (data: {
        address: string | null;
            ...
            >
                ...
                <Paper elevation={2} sx={{ width: "100%", mt: 3, p: 3, borderRadius: 2 }}>
                    <Box sx={{ display: "flex", alignItems: "center", mb: 2 }}>
                        ...
                        <Tooltip
                            title="Connect your wallet to interact with governance actions"
                            arrow
                        >
                ...
                <Paper elevation={3} sx={{ width: "100%", mt: 3, borderRadius: 2 }}>
                    ...
                    >
                        ...
                        />
                        <Tab
                            label={isMobile ? "Poll" : "Internal Poll"}
                            icon={<HowToVote />}
                            iconPosition="start"
                        />
                        <Tab
                            label={isMobile ? "Builder" : "Tx Builder"}
                            icon={<Build />}
                            iconPosition="start"
                        />
                        {/* NEW: Files tab */}
                        <Tab label="Files" icon={<FolderOpen />} iconPosition="start" />
                    </Tabs>
                    ...
                    <TabPanel value={tabValue} index={3}>
                        <InternalVote connectedWalletAddress={stakeCredential} proposals={proposals} />
                    </TabPanel>
                    ...
                    {/* Files Tab Panel (password-protected) */}
                ...
                >
                    <a
                        href="https://github.com/Tingvard-cc/gulating"
                        target="_blank"
